...
(`Trait where Self == Testing.ConditionTrait`) that skips the test when
`CI=true` is set in the environment (always true on GitHub Actions).
...
- A future change to the runner environment (e.g. GitHub Actions exposing
  ...
  short-circuits on `CI=true`, not on capability detection. Switching to
  capability detection would be a follow-up ADR.
